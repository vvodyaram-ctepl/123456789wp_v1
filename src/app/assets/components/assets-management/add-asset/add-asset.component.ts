...
export class AddAssetComponent implements OnInit {
        ...
        this.assetsService.getAssetsService(`/api/assets/${this.id}`).subscribe(res => {
          ...
          if (res.status.success === true) {
            ...
            this.assetsForm.patchValue({
              ...
              "studyName": assetsData.studyId ? this.studyArr.filter((study: any) => study.studyId == assetsData.studyId)[0] : ''
            });
            this.spinner.hide();
          }
        })
  ...
  async getInitialData(): Promise<any> {
    this.spinner.show();
    await this.assetsService.getAssetsService('/api/lookup/getDeviceLocations').toPromise().then(res => {
      if (res.status.success === true) {
    ...
    await this.addUserService.getStudy('/api/study/getStudyList', { params: { includeFuture: 'Yes', includeVirtual: 'No' } }).toPromise().then(res => {
  ...
  submitForm() {
    ...
    if (this.assetsForm.value.studyName.studyId && this.assetsForm.value.statusId != '10') {
      this.toastr.error("Status must be reserved when study is selected.")
      return false;
    }

    if (!this.assetsForm.value.studyName.studyId && this.assetsForm.value.statusId == '10') {
      this.toastr.error("Study must be selected when status is reserved.")
      return false;
    }

    this.submitFlag = true;
    let reqObject = this.assetsForm.getRawValue();
    reqObject["studyId"] = reqObject.studyName.studyId;
    delete reqObject.studyName;
    if (this.editProd) {
  ...
  studySelected() {
    if (this.assetsForm.value.studyName.studyId)
      this.assetsForm.get('statusId').patchValue(10);
  }
