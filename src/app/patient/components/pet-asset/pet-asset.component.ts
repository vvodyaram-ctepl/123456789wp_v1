...
export class PetAssetComponent implements OnInit {
  ...
  getInitialData(): Promise<any> {
    ...
    this.spinner.show();
    let studyIds = [];
    studyIds = this.tabData.petStudy?.arr?.map((study: any) => study.studyName.studyId).toString();
    return this.assetService.getDeviceDetails('/api/assets/getAllDevices', { params: { studyIds } }).toPromise().then(resp => {
