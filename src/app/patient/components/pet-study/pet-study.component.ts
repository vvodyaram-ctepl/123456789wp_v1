...
export class PetStudyComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.spinner.show();
    this.adduserservice.getStudy('/api/study/getStudyList', { params: { includeFuture: 'No', includeVirtual: 'Yes' } }).subscribe(res => {
  ...
  removeItem(i: number) {
    ...
    if (this.editFlag && this.arr.value[i].studyName.studyId) {
      ...
      //for removing the study ass data in device tab when study removed in study tab or To remove devices that are reserved for this study in data streams tab
      if (data.petStudyDevices && data.petStudyDevices.arr) {
        result = data.petStudyDevices.arr.filter(ele => ele.study != studyId && ele.deviceNumber.studyId != studyId);
      }
      result1["arr"] = result;
      this.tabservice.setModelData(result1, 'petStudyDevices');

      // To remove this study reserved assets in asset tab
      if (data.petDevices && data.petDevices.arr) {
        let petDevices = data.petDevices.arr.filter((petDevice: any) => petDevice.deviceNumber.studyId != studyId);
        let obj = Object.assign({});
        obj["arr"] = petDevices;
        this.tabservice.setModelData(obj, 'petDevices');
      }
    }
    this.arr.removeAt(i);
  }
