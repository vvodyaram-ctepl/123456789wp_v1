...
export class PushNotificationStudyComponent implements OnInit {
  ...
  fSelected(i) {
    ...
    if (freq == 'Only Once' && this.addClinicForm.value.arr[i].startDate && this.addClinicForm.value.arr[i].endDate) {
      this.addClinicForm.get('arr')['controls'][i].get('endDate').patchValue(this.addClinicForm.value.arr[i].startDate);
    }
  }
