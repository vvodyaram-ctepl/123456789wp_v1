...
export class StudyImageScoringComponent implements OnInit {
  ...
  fSelected(i) {
    let frequency = this.imageScoringForm.value.arr[i].frequency;
    if (frequency == 1 && this.imageScoringForm.value.arr[i].startDate && this.imageScoringForm.value.arr[i].endDate) {
      this.imageScoringForm.get('arr')['controls'][i].get('endDate').patchValue(this.imageScoringForm.value.arr[i].startDate);
    }
  }
