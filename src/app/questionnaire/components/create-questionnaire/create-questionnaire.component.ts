...
export class CreateQuestionnaireComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  constructor(
    ...
    // needed in both add questions and preview questionnaire
    this.getValidityPeriods();
  }
    ...
    if (this.router.url.includes("/user/questionnaire/edit") && (!this.data || (this.data && !this.data.basicDetails))) {
      ...
      this.questionnaireService.getQuestionnaireById(`/api/questionnaire/${this.editId}`).subscribe(async res => {
        if (res.status.success === true) {
          ...
          ques?.questions?.forEach((q: any) => {
            q.preDefinedQuestion = q.question;
            if (q.questionImageUrl) {
              q.isQuesImg = true;
              q.quesImg = [{ name: q.questionImageName, url: q.questionImageUrl, type: 'image/*' }];
            }

            // To make isOption true for all the options along with "Others, specify and None of the above"
            q.questionAnswerOptions.forEach((o: any) => {
              o.isOption = true;
              if (o.ansOptionMediaUrl) {
                o.isOptImg = true;
                o.optImg = [{ name: o.ansOptionMediaName, url: o.ansOptionMediaUrl, type: 'image/*' }];
                o.ansOptionMediaType = 1;
              }

              // To add qualifying categories from isQualifyingCategory flag
              o.qualifyingCategories = o.answerCategories?.filter((ansCat: any) => ansCat.isQualifyingCategory);
            });
          });
  ...
  getValidityPeriods() {
    this.lookupService.getValidityPeriods().subscribe(res => {
      if (res.status.success === true) {
        this.tabservice.setModelData(res.response.questionValidityPeriods, 'validityPeriods');
      }
      else {
        this.toastr.error(res.errors[0].message);
        this.spinner.hide();
  ...
  questionnaireTypeSelect($event) {
    ...
    if ($event) {
      ...
      if (questionnaireTypeId) {
        this.spinner.show();
        this.lookupService.getCommon(`/api/lookup/getQuestionnaireCategoryList/${questionnaireTypeId}`).subscribe(res => {
          if (res.status.success === true) {
  ...
  async getQuestionnaireCategoryById(id) {
    this.spinner.show();
    this.lookupService.getCommon(`/api/lookup/getQuestionnaireCategoryList/${id}`).subscribe(res => {
      if (res.status.success === true) {
        this.questionnaireCategories = res.response.questionnaireCategories;
      }
      else {
