...
export class AddQuestionsComponent {
  ...
  optionCategoryDropdownSettings = {
    enableCheckAll: true
  };
  categories = [];
  validityPeriods = [];
  ...
  patchData() {
    ...
    // Validity Period
    this.validityPeriods = this.data['validityPeriods'];
    ...
    if (addQuestions.length) {
      for (var i = 0; i < addQuestions.length; i++) {
        ...
        quesCtrl.patchValue({ questionTypeId: ques.questionTypeId });
        ...
        //Need to patch only after typechanged function, otherwise in typechanged function the values gets reset
        quesCtrl.patchValue(onlyQues);

        // To set qualifying categories data
        if (onlyQues.questionCategories.length)
          this.onSelectQuesCategory(i);
        ...
        for (var j = 0; j < ques.questionAnswerOptions.length; j++) {
          ...
          // To set qualifying categories data
          if (opt.answerCategories.length)
            this.onSelectAnsCategory('', j, i);
        }
      }
    }
  ...
  async getInitialData() {
    ...
    this.lookupService.getCategories().subscribe(res => {
      if (res.status.success === true) {
        this.categories = res.response.questionCategories;
      }
      else {
        this.toastr.error(res.errors[0].message);
        this.spinner.hide();
  ...
  typeChanged(value, formName, quesIndx?) {
    ...
    //reset to defaults
    newQuestionCtrl.patchValue({ shuffleOptionOrder: false });
  ...
  editPreDefinedQuestion(question, i) {
    ...
    if (!onlyQues.validityPeriodId) {
      onlyQues.validityPeriodId = '';

      // To make default validity period as Year when includeInDataExtract is selected
      if (onlyQues.isIncludeInDataExtract)
        onlyQues.validityPeriodId = 5;
    }
  ...
  removeQuesAns(formName, ansIndx, quesIndx) {
    let ansArr, quesCtrl;
    if (formName === 'addQues') {
      quesCtrl = this.addQuesForm.get('questions')['controls'][quesIndx];
      quesCtrl.patchValue({ isUpdated: true });
    }
    else {
      quesCtrl = this.editQuesForm;
    }
    ansArr = quesCtrl.get('questionAnswerOptions') as FormArray;
    ...
    //To check for other option categories and patch them to question categories - (indirectly deleting the current unselected option categories from question categories if they are not selected in anyother option)
    // In detail, if abc is selected only for this option, and this whole option is deleted, then abc must be unchecked in question categories as well.(Each question category selected must be selected atleast in an option)
    this.tochangeQuestionCategoriesSelectionBasedOnOptionCategories(quesCtrl);
  }
  ...
  addQues() {
    ...
    if (this.addQuesForm.valid) {
      ...
      // To make default validity period as Year when includeInDataExtract is selected
      if (newQuestionCtrl.value.isIncludeInDataExtract)
        newQuestionCtrl.get('validityPeriodId').patchValue(5);
    }
    else {
      this.toastr.error('Please enter a valid question with valid options', 'Error!');
    }
  ...
  addDefaultOptions(newQuestionCtrl) {
    if (newQuestionCtrl.value.questionTypeId == 1 || newQuestionCtrl.value.questionTypeId == 2 || newQuestionCtrl.value.questionTypeId == 3 || (this.editFlag && !newQuestionCtrl.value.questionTypeId)) {
  ...
  checkIncludeDataExtract() {
    let validityPeriodToBePatched: string | number = '';
    // To make default validity period as Year when includeInDataExtract is selected
    if (this.includeAll)
      validityPeriodToBePatched = 5;
    ...
    this.selectedQuestions.forEach((preDefinedSelectedQues: any) => {
      ...
      // To patch the validityPeriod that is already selected previously
      if (this.includeAll) {
        if (preDefinedSelectedQues.validityPeriodId)
          validityPeriodToBePatched = preDefinedSelectedQues.validityPeriodId;
        else
          validityPeriodToBePatched = 5;
      }
      preDefinedSelectedQues.validityPeriodId = validityPeriodToBePatched;
    });
    ...
    this.editQuesForm.get('validityPeriodId').patchValue(validityPeriodToBePatched);
    ...
    questionCtrls.controls.forEach((ques: any, i: number) => {
      ...
      // To patch the validityPeriod that is already selected previously
      if (this.includeAll) {
        if (ques.value.validityPeriodId)
          validityPeriodToBePatched = ques.value.validityPeriodId;
        else
          validityPeriodToBePatched = 5;
      }
      ques.get('validityPeriodId').patchValue(validityPeriodToBePatched);
  ...
  verifyisAllInclude(index?: number) {
    ...
    if (index >= 0) {
      if (!this.addQuesForm.value.questions[index].isIncludeInDataExtract)
        this.addQuesForm.get('questions')['controls'][index].get('validityPeriodId').patchValue('');
      else // To make default validity period as Year when includeInDataExtract is selected
        this.addQuesForm.get('questions')['controls'][index].get('validityPeriodId').patchValue(5);
    }
    else {
      if (!this.editQuesForm.value.isIncludeInDataExtract)
        this.editQuesForm.get('validityPeriodId').patchValue('');
      else // To make default validity period as Year when includeInDataExtract is selected
        this.editQuesForm.get('validityPeriodId').patchValue(5);
    }
  }

  //On ques categories select/select all
  onSelectQuesCategory(quesIndex?: number, event?: any) {
    let quesControls: any;
    if (quesIndex >= 0) {
      quesControls = this.addQuesForm.controls.questions['controls'][quesIndex];
    }
    else {
      quesControls = this.editQuesForm;
    }

    quesControls.controls.questionAnswerOptions.controls.forEach((quesOpt: any) => {
      if ((this.toUpperAndTrim(quesOpt.value.questionAnswer) != this.noneOfTheAboveOption) && (this.toUpperAndTrim(quesOpt.value.questionAnswer) != this.othersOption)) {
        if (event) {
          let isExist = [];
          if (quesOpt.value.answerCategories.length)
            isExist = quesOpt.value.answerCategories?.filter((ansCategory: any) => ansCategory.questionCategoryId == event.questionCategoryId);
          if (!isExist.length)
            quesOpt.get('answerCategories').patchValue([...quesOpt.value.answerCategories, { ...event }]);
        }
        else
          quesOpt.get('answerCategories').patchValue([...quesControls.value.questionCategories]);


        quesOpt.get('qualifyingCategoriesData').patchValue([...quesOpt.value.answerCategories]);
        // To make the qualifying options selected by default
        quesOpt.get('qualifyingCategories').patchValue([...quesOpt.value.answerCategories]);
      }
    });
  }

  //On ques categories de-select/de-select all
  onDeselectQuesCategory(quesIndex?: number, event?: any) {
    let quesControls: any;
    if (quesIndex >= 0) {
      quesControls = this.addQuesForm.controls.questions['controls'][quesIndex];
    }
    ...
    quesControls.controls.questionAnswerOptions.controls.forEach((quesOpt: any) => {
      if (this.toUpperAndTrim(quesOpt.value.questionAnswer) != this.noneOfTheAboveOption && this.toUpperAndTrim(quesOpt.value.questionAnswer) != this.othersOption) {
        let ansCategories: any = [], existingIndex: number, qualifyingCategoriesSelected: any = [], qualifyingSelectedIndex: number;
        if (event) {
          //To remove from option categories
          quesOpt.value.answerCategories?.forEach((ansCategory: any, index: number) => {
            if (ansCategory.questionCategoryId == event.questionCategoryId)
              existingIndex = index;
          });
          ansCategories = [...quesOpt.value.answerCategories];
          if (existingIndex >= 0)
            ansCategories.splice(existingIndex, 1);

          //To remove from qualifying Categories
          quesOpt.value.qualifyingCategories?.forEach((quaCat: any, index: number) => {
            if (quaCat.questionCategoryId == event.questionCategoryId)
              qualifyingSelectedIndex = index;
          });
          qualifyingCategoriesSelected = [...quesOpt.value.qualifyingCategories];
          if (qualifyingSelectedIndex >= 0)
            qualifyingCategoriesSelected.splice(qualifyingSelectedIndex, 1);
        }
        else {
          quesControls.get('questionCategories').patchValue([]);
        }

        quesOpt.patchValue({ answerCategories: [...ansCategories], qualifyingCategoriesData: [...ansCategories], qualifyingCategories: [...qualifyingCategoriesSelected] });
      }
    });
  }

  onSelectAnsCategory(event: any, optionIndex: number, quesIndex?: number) {
    let quesControls: any, optControls: any;
    if (quesIndex >= 0) {
      quesControls = this.addQuesForm.controls.questions['controls'][quesIndex];
    }
    else {
    ...
    optControls = quesControls.get('questionAnswerOptions')['controls'][optionIndex];

    // To make the changes to question categories based on option categories select or unselect
    optControls.value.answerCategories?.forEach((ansCategory: any) => {
      if (quesControls.value.questionCategories) {
        let isExist = quesControls.value.questionCategories.filter((quesCategory: any) => quesCategory.questionCategoryId == ansCategory.questionCategoryId);
        if (!isExist.length) {
          quesControls.get('questionCategories').patchValue([...quesControls.value.questionCategories, { ...ansCategory }]);
        }
      }
      else {
        quesControls.get('questionCategories').patchValue([{ ...ansCategory }]);
      }
    });

    // To make the changes to qualifying categories data array based on option categories select or unselect
    optControls.get('qualifyingCategoriesData').patchValue([...optControls.value.answerCategories]);

    // To select qualifying categories by default based on option categories selection
    let qualifyingCategoriesSelected = [...optControls.value.qualifyingCategories];
    if (event && event.length) {
      qualifyingCategoriesSelected = [...optControls.value.answerCategories];
    }
    else if (event) {
      qualifyingCategoriesSelected.push({ ...event });
    }
    optControls.get('qualifyingCategories').patchValue(qualifyingCategoriesSelected);
  }

  onDeselectAnsCategory(event: any, optionIndex: number, quesIndex?: number) {
    let quesControls: any, optControls: any;
    if (quesIndex >= 0) {
      quesControls = this.addQuesForm.controls.questions['controls'][quesIndex];
    }
    ...
    if (event) {
      // To check whether this category is existed for other options, if not we need to deselect it from question categories
      let isExistInOtherOptions: boolean = false;
      quesControls.value.questionAnswerOptions.forEach((ansOption: any, index: number) => {
        if (index != optionIndex && ansOption.answerCategories.length) {
          let arr = ansOption.answerCategories?.filter((ansCat: any) => ansCat.questionCategoryId == event.questionCategoryId && index != optionIndex);
          isExistInOtherOptions = arr.length;
        }
      });
      let categories = [...quesControls.value.questionCategories];
      if (!isExistInOtherOptions) {
        quesControls.value.questionCategories?.forEach((quesCat: any, quesCatIndex: number) => {
          if (quesCat.questionCategoryId == event.questionCategoryId)
            categories.splice(quesCatIndex, 1);
        });
        quesControls.get('questionCategories').patchValue(categories);
      }
    }
    else {
      optControls.get('answerCategories').patchValue([]);
      //To check for other option categories and patch them to question categories - (indirectly deleting the current unselected option categories from question categories if they are not selected in anyother option)
      // In detail, if abc is selected only for this option, and this is now unselected, then abc must be unchecked in question categories as well.(Each question category selected must be selected atleast in an option)
      this.tochangeQuestionCategoriesSelectionBasedOnOptionCategories(quesControls);
    }

    if (optControls.value.answerCategories)
      optControls.get('qualifyingCategoriesData').patchValue([...optControls.value.answerCategories]);
    else
      optControls.get('qualifyingCategoriesData').patchValue([]);

    //To remove the qualifying categories if they are deselected from answer categories
    const filteredArray = optControls.value.qualifyingCategoriesData.filter(value => optControls.value.qualifyingCategories.includes(value));
    optControls.get('qualifyingCategories').patchValue(filteredArray);
  }

  tochangeQuestionCategoriesSelectionBasedOnOptionCategories(quesControls: any) {
    let quesCategoriesToBePatched = [];
    quesControls.value.questionAnswerOptions.forEach((ansOption: any) => {
      if (ansOption.answerCategories) {
        ansOption.answerCategories?.forEach((ansCat: any) => {
          let isExist = quesCategoriesToBePatched.filter((cat: any) => cat.questionCategoryId == ansCat.questionCategoryId);
          if (!isExist || !isExist.length)
            quesCategoriesToBePatched.push({ ...ansCat });
        });
      }
    });
    if (quesControls.value.questionCategories.length != quesCategoriesToBePatched.length)
      quesControls.get('questionCategories').patchValue([...quesCategoriesToBePatched]);
  }
