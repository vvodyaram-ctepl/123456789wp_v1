...
export class PreviewQuestionnaireComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.data.questions.forEach((question: any, i: number) => {
      ...
      // To show validity text
      if (this.data.questions[i].validityPeriodId) {
        let validityPeriods = this.data.validityPeriods;
        this.data.questions[i].validityPeriod = validityPeriods.filter((validity: any) => validity.validityPeriodId == this.data.questions[i].validityPeriodId)[0].validityDescription;
      }
    });
  ...
  addQuestionnaire(hasSkipImplementation?: boolean) {
    ...
    finalQuestions.forEach((question: any) => {
      ...
      //To add the qualifying category flag to ans categories and remove qualifying categories array
      question.questionAnswerOptions.forEach((quesOpt: any) => {
        if (quesOpt.answerCategories) {
          quesOpt?.answerCategories.forEach((ansCat: any) => {
            ansCat.isQualifyingCategory = false;
            if (quesOpt.qualifyingCategories) {
              let isQualified = quesOpt.qualifyingCategories.filter((qualifyingCat: any) => qualifyingCat.questionCategoryId == ansCat.questionCategoryId);
              if (isQualified.length)
                ansCat.isQualifyingCategory = true;
            }
          });
        }
        delete quesOpt.qualifyingCategories;
        delete quesOpt.qualifyingCategoriesData;
      });
