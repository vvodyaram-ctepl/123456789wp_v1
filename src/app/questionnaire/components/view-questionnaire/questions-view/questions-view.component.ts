      ...
      this.questionnaireService.getQuestionnaireById(`/api/questionnaire/${this.questionnaireId}`).subscribe(res => {
        if (res.status.success === true) {
          ...
          this.data.questions.forEach((question: any, i: number) => {
            ...
            // To get the qualifying categories array
            question.questionAnswerOptions.forEach((ansOpt: any) => {
              ansOpt.qualifyingCategories = ansOpt.answerCategories.filter((ansCat: any) => ansCat.isQualifyingCategory);
            });
          });
