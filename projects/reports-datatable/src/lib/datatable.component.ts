    ...
    if (this.filterTypeArr) {
      this.filterTypeArr.forEach(flr => {
        ...
        if (flr.id === 'assetStatus') {
          let url = '/api/lookup/getDeviceStatuses';
          this.lookupService.getCommon('/api/lookup/getDeviceStatuses', { params: { showOnlyStudyStatuses: (this.reportName == 'Asset Tracking Report' || this.reportName == 'Study Based Report') ? 'Yes' : 'No' } }).subscribe(res => {
            // this.filterValue = "";
